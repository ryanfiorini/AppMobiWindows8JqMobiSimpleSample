    ...
    var css3Animate = (function () {
        ...
        var css3Animate = function (elID, options) {
				...
				if(options["time"]==0 && options["callback"]){
					...
					this.el.addEventListener(transitionEnd, that.finishAnimation, false);
				}
            };
 ...
 */ 
 (function($) {
	...
	function bindTouchLayer(){
		...
		if(jq.os.android && !jq.os.chrome&&!jq.os.desktop) {
    ...
    var scroller = (function() {
		...
		nativeScroller.prototype.hideRefresh=function(animate){
			...
			var endAnimationCb = function(canceled){
					if(!canceled){	//not sure if this should be the correct logic....
						that.el.style[$.cssPrefix+"transform"]="none";
						that.el.style[$.cssPrefix+"transition-property"]="none";
						that.el.scrollTop=0;
        ...
        function createScrollBar(width, height) {
            ...
            scrollDiv.style[$.cssPrefix+"border-radius"] = "2px";
            scrollDiv.style.opacity = 0;
        ...
        jsScroller.prototype.hideScrollbars=function() {
            if (this.hscrollBar)
            {
                ...
                this.hscrollBar.style[$.cssPrefix+"transition-duration"] = "0ms";
            }
            if (this.vscrollBar){
                this.vscrollBar.style.opacity = 0
                this.vscrollBar.style[$.cssPrefix+"transition-duration"] = "0ms";
            }
        }
        ...
        jsScroller.prototype.onTouchStart=function(event) {
			...
			if(this.setVScrollBar(scrollInfo, 0, 0)){
	            ...
	            this.vscrollBar.style[$.cssPrefix+"transition"] = '';
			...
			if(this.setHScrollBar(scrollInfo, 0, 0)){
                ...
                this.hscrollBar.style[$.cssPrefix+"transition"] = ''; 
		...
		jsScroller.prototype.startFormsMode = function(){
			...
			//set new css rules
			this.el.style[$.cssPrefix+"transform"] = "none";
			this.el.style[$.cssPrefix+"transition"] = "none";
			this.el.style[$.cssPrefix+"perspective"] = "none";
			
			//set position
            this.scrollerMoveCSS({x:numOnly(cssMatrix.e),y:numOnly(cssMatrix.f)}, 0);
			
			//container
			this.container.style[$.cssPrefix+"perspective"] = "none";
			this.container.style[$.cssPrefix+"backface-visibility"] = "visible";
			//scrollbars
			if(this.vscrollBar){
				this.vscrollBar.style[$.cssPrefix+"transform"] = "none";
				this.vscrollBar.style[$.cssPrefix+"transition"] = "none";
				this.vscrollBar.style[$.cssPrefix+"perspective"] = "none";
				this.vscrollBar.style[$.cssPrefix+"backface-visibility"] = "visible";
			}
			if(this.hscrollBar){
				this.hscrollBar.style[$.cssPrefix+"transform"] = "none";
				this.hscrollBar.style[$.cssPrefix+"transition"] = "none";
				this.hscrollBar.style[$.cssPrefix+"perspective"] = "none";
				this.hscrollBar.style[$.cssPrefix+"backface-visibility"] = "visible";
			}
		...
		jsScroller.prototype.stopFormsMode = function(){
			...
			this.androidFormsMode=false;
			//set new css rules
			this.el.style[$.cssPrefix+"perspective"] = 1000;
            this.el.style.marginTop = 0;
            this.el.style.marginLeft = 0;
			this.el.style[$.cssPrefix+"transition"] = '0ms linear';	//reactivate transitions
			//set position
			this.scrollerMoveCSS({x:numOnly(cssMatrix.e),y:numOnly(cssMatrix.f)}, 0);
			//container
			this.container.style[$.cssPrefix+"perspective"] = 1000;
			this.container.style[$.cssPrefix+"backface-visibility"] = "hidden";
			//scrollbars
			if(this.vscrollBar){
				this.vscrollBar.style[$.cssPrefix+"perspective"] = 1000;
				this.vscrollBar.style[$.cssPrefix+"backface-visibility"] = "hidden";
			}
			if(this.hscrollBar){
				this.hscrollBar.style[$.cssPrefix+"perspective"] = 1000;
				this.hscrollBar.style[$.cssPrefix+"backface-visibility"] = "hidden";
			}
			...
			if(this.el && this.el.style){
				...
				if(this.eventsActive){
					if(this.androidFormsMode){
			            ...
			            this.el.style[$.cssPrefix+"transform"] = "translate" + translateOpen + distanceToMove.x + "px," + distanceToMove.y + "px" + translateClose;
			            this.el.style[$.cssPrefix+"transition-duration"]= time + "ms";
			            this.el.style[$.cssPrefix+"transition-timing-function"] = timingFunction;
					}
				}
...
(function ($) {
    ...
    passwordBox.prototype = {
        ...
        getOldPasswords: function (elID) {
            ...
            if ($.os.webkit == false) return; //Allow for webkit right now
    ...
    $['selectBox'] = {
        ...
        getOldSelects: function(elID) {
            if (!$.os.android || $.os.androidICS)
               return;
            if (!$.fn['scroller']) {
                alert("This library requires jq.Scroller");
                return;
            }
            var container = elID && document.getElementById(elID) ? document.getElementById(elID) : document;
            if (!container) {
                alert("Could not find container element for jq.selectBox " + elID);
                return;
            }
    ...
    ui.prototype = {
        ...
        */
        ...
        clearAnimations: function(inViewDiv) {
			inViewDiv.style[$.cssPrefix+"transform"] = "none";
			inViewDiv.style[$.cssPrefix+"transition"] = "none";
        }
